...
## Infrascope Tech Stack (reference for all architecture decisions)
...
| Config Mgmt | SaltStack (Salt Master → Minions)                  |
| Provisioning| Ironic (bare metal)                                |
| Identity    | Keycloak + 389ds (LDAP)                            |
| Monitoring  | Prometheus + Alertmanager + OpenSearch              |
| Secrets     | HashiCorp Vault                                    |
| Networking  | OVN + OVS                                          |
| Web Server  | Nginx                                              |
| OS          | Rocky Linux 9                                      |

When reviewing architecture, always consider:
- Does this go through CoreService or directly to the target?
- Is the Salt State idempotent? What happens on re-run?
- How does this interact with Keycloak auth boundaries?
- What Prometheus metrics and OpenSearch logs does this produce?
- What happens if the Salt Minion is unreachable?
...
### Salt State Architecture Checklist
For any feature that touches Salt:
* Draw the Pillar → State → Minion data flow
* Identify which Grains/Pillars are used for targeting
* Verify state ordering (require/watch/onchanges)
* Map failure modes: minion offline, partial apply, conflicting states
* Check for Fluent-Bit/SSL/SSH conflicts (known pain point)
