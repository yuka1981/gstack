...
1. Check the current branch. If on `develop`, **abort**: "You're on develop. Ship from a feature branch."

2. Run `git status` (never use `-uall`). Uncommitted changes are always included — no need to ask.

3. Run `git diff develop...HEAD --stat` and `git log develop..HEAD --oneline` to understand what's being shipped.

---

## Step 2: Merge origin/develop (BEFORE tests)

Fetch and merge `origin/develop` into the feature branch so tests run against the merged state:
...
Run the Infrascope test suites in parallel:

```bash
# Rails tests
bundle exec rspec 2>&1 | tee /tmp/ship_rspec.txt &

# Frontend tests (React)
yarn test --run 2>&1 | tee /tmp/ship_frontend.txt &

# Salt state lint (if Salt files changed)
if git diff origin/develop --name-only | grep -q '\.sls'; then
  salt-lint salt/ 2>&1 | tee /tmp/ship_salt_lint.txt &
fi
...
2. Run `git diff origin/develop` to get the full diff (scoped to feature changes against the freshly-fetched remote develop).
...
2. **Auto-decide the bump level based on the diff:**
   - Count lines changed (`git diff origin/develop...HEAD --stat | tail -1`)
...
2. Auto-generate the entry from **ALL commits on the branch** (not just recent ones):
   - Use `git log develop..HEAD --oneline` to see every commit being shipped
   - Use `git diff develop...HEAD` to see the full diff against develop
...
- `git diff develop...HEAD` (full diff against develop)
- `git log develop..HEAD --oneline` (all commits being shipped)
...
## Step 7.5: SQA Gate Reminder

Before creating the PR, output:

> **SQA Gate:** This PR requires SQA sign-off before merge to `develop`.
> Assign the PR to the SQA team member for review after creation.

If the branch name contains `hotfix/` or `urgent/`, note that SQA can review post-merge but must still sign off within 24 hours.
...
- [ ] Salt states changed: {list affected .sls files, or "None"}
- [ ] Database migration: {yes/no, reversible?}
- [ ] New Prometheus metrics: {list, or "None"}
- [ ] New OpenSearch indices: {list, or "None"}
- [ ] Keycloak/389ds changes: {describe, or "None"}
- [ ] Network (OVN/OVS) changes: {describe, or "None"}

## Rollback Plan
{describe how to revert — git revert? feature flag? Salt state rollback? migration down?}
...
## SQA
- [ ] SQA team member assigned
- [ ] Test cases documented
- [ ] Verified on staging environment

## Test Results
- [x] RSpec: {N examples, 0 failures}
- [x] Frontend: {N tests passed}
- [x] Salt lint: {pass/skip}
...
- **Always include Infrastructure Impact in the PR body.** Scan the diff for .sls, migration, prometheus, opensearch changes.
- **Always output the SQA Gate reminder** before creating the PR.
- **Salt lint is mandatory** when .sls files are in the diff.
